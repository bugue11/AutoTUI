class ZTable extends ZComponent {
	...
	build() {
			...
			this.view.find("th").each(function() {
					...
					if (th.data("z-col")) {
							...
							if (th.text()) thisComponent.nameTH[thisComponent.cols.length - 1] = th.text();
							if (th.data("z-columna-titulo")) thisComponent.columnaTitulo[thisComponent.cols.length - 1] = true;

					} else {
							console.error("No data-z-col in <th> for table '" + thisComponent.view.prop("id") + "'");
					}
			});
			if(this.view.data("z-responsive")){
				this.cols.push("-responsive-");
				this.view.find("thead").find("tr").append("<th class='text-right " + (this.editColHeaderClass?this.editColHeaderClass:"") + "'></th>");   			
			}else if (this.view.data("z-edit")) {
					this.cols.push("-edit-");
					this.view.find("thead").find("tr").append("<th class='text-right " + (this.editColHeaderClass?this.editColHeaderClass:"") + "'></th>");            
			}       
	...
	if (this.getDetailsComponent()) {
		this.getDetailsComponent().html("<i class='fa fa-spinner fa-spin fa-2x'></i>");
	}
	}
	...
	ResponsiveColumnShow(rowIndex, row) {
		this.rows[rowIndex] = row;
		this.indexColumna = rowIndex;
		this.mostrarColumnas = true;
		this.clearTable();
		this.paint();
	}

	ResponsiveColumnHide(rowIndex, row) {
		this.rows[rowIndex] = row;
		this.indexColumna = rowIndex;
		...
		for (var i=startRow; i<endRow; i++) {
			...
			$.each(this.cols, function (j, col) {
					if (!thisComponent.hiddenCols[col]) {
						if(col == "-responsive-"){
							html += "<td class='text-center " + (thisComponent.editColCellClass?thisComponent.editColCellClass:"") + "'>";
							if (thisComponent.mostrarColumnas && i == thisComponent.indexColumna){
								html += "<button class='ResponsiveColumnHide btn btn-success btn-round px-2 py-1' data-index='" + i + "' style='cursor: pointer;'><i class='material-icons pull-xs-right'>zoom_out</i></button>"; 
							}else{
								html += "<button class='ResponsiveColumnShow btn btn-success btn-round px-2 py-1' data-index='" + i + "' style='cursor: pointer;'><i class='material-icons pull-xs-right'>zoom_in</i></button>";
							}
							var extra = row._extraEditControls;
							if (extra) {
								$.each(extra, function (k, c) {
									html += "<button class='extra-edit-control btn btn-info btn-round px-2 py-1' data-index='" + i + "' data-name='" + c.name + "' style='cursor: pointer;'><i class='" + c.icon + " ml-1'></i></button>";
								});
							}
							html += "</td>";
						}else if (col == "-edit-") {
							html += "<td class='text-center " + (thisComponent.editColCellClass?thisComponent.editColCellClass:"") + "'>";
							html += "<button class='editador btn-sm btn-info btn-round px-2 py-1' data-index='" + i + "' style='cursor: pointer;'><i class='fa fa-edit'></i></button>";
							html += "<button class='eliminador btn-sm btn-danger btn-round px-2 py-1' data-index='" + i + "' style='cursor: pointer;'><i class='fa fa-trash'></i></button>";
							if (thisComponent.mostrarColumnas && i == thisComponent.indexColumna){
								html += "<button class='ResponsiveColumnHide btn btn-success btn-round px-2 py-1' data-index='" + i + "' style='cursor: pointer;'><i class='material-icons pull-xs-right'>zoom_out</i></button>"; 
							}else{
								html += "<button class='ResponsiveColumnShow btn btn-success btn-round px-2 py-1' data-index='" + i + "' style='cursor: pointer;'><i class='material-icons pull-xs-right'>zoom_in</i></button>";
							...
							let classResponsive = "";
							if (thisComponent.mostrarColumnas && i == thisComponent.indexColumna) {
								classResponsive = " showTD";
							}else {
								if (thisComponent.columnaTitulo[j])classResponsive = " showTD";
								else classResponsive = " hiddenTD";
							}	
							html += "<td data-title='" + thisComponent.nameTH[j] + "'" + (cellClass ? " class='" + cellClass + classResponsive + "'" : "") + ">" + val + " </td>";
						}
					}
			});
			html += "</tr>";
	...
	this.view.find("tbody").find(".ResponsiveColumnShow").click(function (e) {
		e.preventDefault();
		var idx = parseInt($(this).data("index"));
		var row = thisComponent.rows[idx];
		thisComponent.triggerEvent("ResponsiveColumnShow", [idx, row]);
	});

	this.view.find("tbody").find(".ResponsiveColumnHide").click(function (e) {
		e.preventDefault();
		var idx = parseInt($(this).data("index"));
		var row = thisComponent.rows[idx];
		thisComponent.triggerEvent("ResponsiveColumnHide", [idx, row]);
	});
