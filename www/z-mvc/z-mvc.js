...
class ZSelect extends ZComponent {
    ...
    get val() {
        ...
        if (idx >= 0) return this._rows[idx][this.idField];
        else return null;
    }
    set val(v) {
    ...
    setRows(rows, selectedId) {
        this._rows = rows;       
        let html = rows.reduce((st, row, i) => {
            return st + "<option value='" + i + "' " + (selectedId !== undefined && row[this.idField] == selectedId?"selected":"") + ">" + row[this.labelField] + "</option>";
        ...
        return new Promise((onOk, onError) => {
            ...
            zLoadComponent(finalUrl, this)
                .then(({html, controllerClass}) => {
                    ...
                    this.view.html(html);
                    let subview = $(this.view.children()[0]);
                    if (!subview.prop("id")) subview.prop("id", this.id + "_active");
                    var exports = {};
                    try {
                        eval(controllerClass + "\nexports.customController = CustomController;");
                    } catch(error) {
                        console.error("loading controller in", finalUrl);
                        console.error(error);
                        onError(error);
                        return;
                    }
